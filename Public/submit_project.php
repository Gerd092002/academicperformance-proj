...
// Check project exists with subject name
$stmt = $pdo->prepare("SELECT p.*, s.name as subject_name FROM projects p JOIN subjects s ON p.subject_id = s.id WHERE p.id = ?");
...
$stmt = $pdo->prepare("SELECT * FROM submissions WHERE student_id = ? AND project_id = ? ORDER BY created_at DESC LIMIT 1");
$stmt->execute([$student_id, $project_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!empty($_FILES['submission_file']['name'])) {
        ...
        // Validate file type
        $allowed_types = ['pdf', 'doc', 'docx', 'ppt', 'pptx', 'zip', 'rar', '7z', 'tar', 'gz', 'jpg', 'jpeg', 'png', 'mp4', 'mov', 'avi', 'wmv'];
        ...
        if (!in_array($file_ext, $allowed_types)) {
            ...
            $message_type = 'error';
        } elseif ($_FILES['submission_file']['size'] > 50 * 1024 * 1024) { // 50MB limit for projects
            $message = "File size too large. Maximum size is 50MB.";
            $message_type = 'error';
        } else {
            $file_name = $student_id . '_' . $project_id . '_' . time() . '.' . $file_ext;
            ...
            if (move_uploaded_file($_FILES['submission_file']['tmp_name'], $target_file)) {
                if ($existing_submission) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO submissions (student_id, project_id, file_path, created_at) VALUES (?, ?, ?, NOW())");
                    $stmt->execute([$student_id, $project_id, $file_name]);
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .phase-indicator.current::before {
...
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-5xl">
        ...
        <div class="mb-8 text-center">
            <h1 class="text-4xl font-bold text-white mb-2">Project Submission</h1>
            <p class="text-white/80">Submit your comprehensive project work</p>
        </div>
                    ...
                    <div>
                        <div class="flex items-center mb-3">
                            ...
                            <div>
                                ...
                                <p class="text-purple-100"><?= htmlspecialchars($project['subject_name']) ?></p>
                            </div>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-sm opacity-90">Project Complexity</div>
                        <div class="text-3xl font-bold">High</div>
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        <div class="bg-gradient-to-r from-gray-50 to-gray-100 p-6 rounded-2xl border border-gray-200">
                            ...
                            <div class="space-y-4">
                                <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                    ...
                                    <span class="font-semibold text-gray-800"><?= htmlspecialchars($project['subject_name']) ?></span>
                                </div>
                                ...
                                <div class="flex justify-between items-center py-2 border-b border-gray-100">
                                    ...
                                    <div class="text-right">
                                        <span class="font-semibold <?= $is_overdue ? 'text-red-600' : 'text-purple-600' ?>">
                                            <?= date('M d, Y', $due_date) ?>
                                        </span>
                                        <div class="mt-1">
                                            <?php if($is_overdue): ?>
                                            ...
                                            <?php else: ?>
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800 due-date-badge">
                                ...
                                <div class="pt-2">
                                    ...
                                    <div class="bg-white p-4 rounded-lg border border-gray-200 text-gray-700 max-h-60 overflow-y-auto">
                        ...
                        <div class="bg-gradient-to-r from-blue-50 to-cyan-50 p-6 rounded-2xl border border-blue-200">
                            ...
                            <div class="space-y-3">
                                <div class="phase-indicator completed">
                                    <span class="font-medium text-gray-800">Project Proposal</span>
                                    <span class="text-sm text-green-600 ml-2">✓ Completed</span>
                                </div>
                                <div class="phase-indicator completed">
                                    <span class="font-medium text-gray-800">Research & Planning</span>
                                    <span class="text-sm text-green-600 ml-2">✓ Completed</span>
                                </div>
                                <div class="phase-indicator completed">
                                    <span class="font-medium text-gray-800">Development Phase</span>
                                    <span class="text-sm text-green-600 ml-2">✓ Completed</span>
                                </div>
                                <div class="phase-indicator current">
                                    <span class="font-medium text-gray-800">Final Submission</span>
                                    <span class="text-sm text-yellow-600 ml-2">In Progress</span>
                                </div>
                                <div class="phase-indicator">
                                    <span class="font-medium text-gray-800">Presentation & Review</span>
                        ...
                        <div class="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-2xl border border-purple-200">
                            ...
                            <div class="file-preview bg-white p-4 rounded-xl border border-purple-200">
                                <div class="flex items-center">
                                    <div class="file-icon bg-gradient-to-r from-purple-100 to-pink-200">
                            ...
                            <div class="mt-4 text-sm text-purple-700 bg-purple-100 p-3 rounded-lg">
                                <i class="fas fa-info-circle mr-2"></i>
                                Uploading a new file will replace your previous project submission.
                            </div>
                        </div>
                        <?php endif; ?>
                    </div>
                            ...
                            <form method="POST" enctype="multipart/form-data" id="uploadForm" class="space-y-6">
                                ...
                                <div id="fileUploadArea" class="file-upload-area p-8 text-center cursor-pointer transition-all duration-300">
                                    ...
                                    <div id="uploadIcon" class="mx-auto mb-4">
                                        <div class="w-20 h-20 bg-gradient-to-r from-purple-100 to-pink-100 rounded-full flex items-center justify-center mx-auto pulse-animation">
                                            <i class="fas fa-cloud-upload-alt text-purple-600 text-3xl"></i>
                                        </div>
                                    </div>
                                    
                                    <div id="uploadText" class="space-y-2">
                                        <p class="text-lg font-semibold text-gray-700">Drag & drop your project files</p>
                                        <p class="text-gray-500">or click to browse</p>
                                        <p class="text-sm text-gray-400 mt-4">Supported formats: PDF, DOC, PPT, ZIP, Images, Videos</p>
                                        <p class="text-sm text-gray-400">Max file size: 50MB</p>
                                    </div>
                                    ...
                                    <div id="filePreview" class="hidden mt-6">
                                        <div class="bg-white p-4 rounded-xl border border-purple-200 shadow-sm">
                                            <div class="flex items-center">
                                                <div class="file-icon bg-gradient-to-r from-purple-100 to-pink-200">
                                                    <i id="fileIcon" class="fas fa-file text-purple-600 text-xl"></i>
                                                </div>
                                ...
                                <div id="progressContainer" class="hidden">
                                    <div class="flex justify-between items-center mb-2">
                                        ...
                                        <span id="progressPercent" class="font-bold text-purple-600">0%</span>
                                    </div>
                                    <div class="h-2 bg-gray-200 rounded-full overflow-hidden">
                                        <div id="progressBar" class="h-full bg-gradient-to-r from-purple-500 to-pink-500 rounded-full transition-all duration-300" style="width: 0%"></div>
                                    </div>
                                </div>

                                <!-- Multiple Files Note -->
                                <div class="text-sm text-gray-600 bg-gray-50 p-4 rounded-lg border border-gray-200">
                                    <div class="flex items-start">
                                        <i class="fas fa-lightbulb text-yellow-500 mt-1 mr-3"></i>
                                        <div>
                                            <p class="font-medium text-gray-800 mb-1">Compressed Files Recommended</p>
                                            <p class="text-gray-600">For multiple files, compress them into a ZIP or RAR archive before uploading.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- Submit Button -->
                                <div class="pt-4">
                                    <button type="submit" id="submitBtn" class="btn-glow w-full bg-gradient-to-r from-purple-600 to-pink-600 text-white px-6 py-4 rounded-xl font-semibold text-lg hover:from-purple-700 hover:to-pink-700 transition disabled:opacity-50 disabled:cursor-not-allowed">
                                        <i class="fas fa-rocket mr-3"></i>
                                        <?= $existing_submission ? 'Resubmit Project' : 'Submit Final Project' ?>
                                    </button>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="bg-gradient-to-r from-green-50 to-emerald-50 p-6 rounded-2xl border border-green-200">
                            ...
                            <div class="space-y-3" id="checklist">
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    ...
                                    <span class="text-gray-700">All required files included</span>
                                </label>
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input type="checkbox" class="rounded text-purple-600 focus:ring-purple-500">
                                    <span class="text-gray-700">Proper file naming convention</span>
                                </label>
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input type="checkbox" class="rounded text-purple-600 focus:ring-purple-500">
                                ...
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input type="checkbox" class="rounded text-purple-600 focus:ring-purple-500">
                                    <span class="text-gray-700">Sources properly cited</span>
                                </label>
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input type="checkbox" class="rounded text-purple-600 focus:ring-purple-500">
                                    <span class="text-gray-700">Final review completed</span>
                                </label>
                            </div>
                            <div class="mt-4 text-center">
                                <button onclick="checkAll()" class="text-sm text-purple-600 hover:text-purple-800 font-medium">
                        ...
                        <div class="bg-gradient-to-r from-indigo-50 to-blue-50 p-6 rounded-2xl border border-indigo-200">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="text-center p-4 bg-white rounded-xl border border-indigo-100">
                                    <div class="text-2xl font-bold text-purple-600">50MB</div>
                                    <div class="text-sm text-gray-600 mt-1">Max Size</div>
                                </div>
                                <div class="text-center p-4 bg-white rounded-xl border border-indigo-100">
                                    <div class="text-2xl font-bold <?= $existing_submission ? 'text-green-600' : 'text-yellow-600' ?>">
                                        <?= $existing_submission ? 'Submitted' : 'Pending' ?>
                                    </div>
                            ...
                            <div class="mt-4 text-center text-sm text-gray-500">
                                ...
                                Projects are comprehensive submissions
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function handleFileSelection(file) {
            if (file.size > 50 * 1024 * 1024) {
                alert('File size exceeds 50MB limit for projects');
                return;
            }

            const fileExt = file.name.split('.').pop().toLowerCase();
            const allowedExts = ['pdf', 'doc', 'docx', 'ppt', 'pptx', 'zip', 'rar', '7z', 'tar', 'gz', 'jpg', 'jpeg', 'png', 'mp4', 'mov', 'avi', 'wmv'];
            
            if (!allowedExts.includes(fileExt)) {
                alert('File type not allowed. Please upload a valid project file.');
                return;
            }
            ...
            fileIcon.className = 'fas ' + (fileIcons[fileExt] || 'fa-file') + ' text-purple-600 text-xl';
        ...
        document.getElementById('uploadForm').addEventListener('submit', function(e) {
            ...
            const interval = setInterval(() => {
                progress += Math.random() * 15; // Slower progress for larger files
                if (progress > 90) {
        ...
        function downloadProjectBrief() {
            const brief = `PROJECT: <?= addslashes($project['name']) ?>\nSUBJECT: <?= addslashes($project['subject_name']) ?>\nDEADLINE: <?= $project['due_date'] ? date('M d, Y', strtotime($project['due_date'])) : 'Not specified' ?>\n\nPROJECT DESCRIPTION:\n<?= addslashes($project['description']) ?>\n\nSUBMISSION REQUIREMENTS:\n1. Complete project documentation\n2. Source code/files (if applicable)\n3. Presentation materials\n4. Any supporting documents\n5. Proper citations and references\n\nNOTES:\n- Maximum file size: 50MB\n- Compress multiple files into ZIP/RAR\n- Include your name and student ID\n- Submit before the deadline`;
            
            const blob = new Blob([brief], { type: 'text/plain' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'Project_Brief_<?= preg_replace('/[^A-Za-z0-9]/', '_', $project['name']) ?>.txt';
        ...
        function viewProjectSubmission() {
            alert('Project submission viewing feature. In full implementation, this would open or download the submitted project files.');
        }
        ...
        function saveAsDraft() {
            ...
            if (!file) {
                alert('Please select a file first to save as draft.');
                return;
            }
            
            alert('Draft saved! This feature would save your progress without final submission.');
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            checklist.addEventListener('change', function(e) {
                if (e.target.type === 'checkbox') {
                    const allChecked = Array.from(checklist.querySelectorAll('input[type="checkbox"]'))
                        .every(checkbox => checkbox.checked);
                    
                    if (allChecked) {
                        checklist.parentElement.classList.add('border-green-300', 'bg-green-50');
                    } else {
                        checklist.parentElement.classList.remove('border-green-300', 'bg-green-50');
                    }
                }
            });
        });
